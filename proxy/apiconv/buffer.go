...
// BufferingWriter wraps a gin.ResponseWriter and captures the upstream response
// in memory instead of forwarding it, so a non-streaming reply can be read in
// full, translated, and re-emitted in the client's format. It mirrors the
// proven writer used by the ollama compatibility layer (proxy/ollama/buffer.go);
// kept here so apiconv carries no dependency on that package.
//
...
func (b *BufferingWriter) Size() int {
	if b.committed {
...
func (b *BufferingWriter) Written() bool {
	if b.committed {
		return b.ResponseWriter.Written()
	}
	return b.headerCaptured
}

func (b *BufferingWriter) WriteHeaderNow() {
	if b.committed {
