...
// findInBodyMetrics returns the first existing per-request metrics object at
// one of the known paths in the parsed JSON. Returns a zero gjson.Result when
// none is present.
...
func parseMetrics(modelID string, start time.Time, usage, timings, inBodyMetrics gjson.Result) (ActivityLogEntry, error) {
	...
	// Non-streaming requests have no meaningful TTFT signal; pass zero firstWrite
	// so buildMetrics falls through to the non-streaming approximation tier.
...
// derives per-request rates using a four-tier fallback:
//  1. Upstream-emitted timings block (llama-server /v1/chat/completions).
//  2. Opportunistic in-body metrics object (e.g., vLLM-style metrics).
//  3. Streaming TTFT split — when firstWrite is set, split duration into
