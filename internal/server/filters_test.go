...
func TestServer_ApplyFilters(t *testing.T) {
	...
	t.Run("set-if-undefined setParamsByID is a no-op when setParams already set the key", func(t *testing.T) {
		// Filters apply like a pipe: stripParams | setParamsByMatch |
		// setParams | setParamsByID. A "?" key only fills what is undefined
		// at its stage, so a value an earlier stage set counts as defined.
		f := config.Filters{
